    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active">KSD</li>
        </ol>
      </div>
    </div>
  </div><!-- /.container-fluid -->
...
<section class="content">
  ...
  <div class="card">
    <div class="card-header with-border">
      <h3 class="card-title">List KSD</h3>

      <div class="card-tools pull-right">
        <!-- <?php if (hasPermissions('permissions_add')): ?> -->
          <a href="<?php echo url('KSD/add') ?>" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> Tambah KSD</a>
        <!-- <?php endif ?> -->
      </div>
            ...
            <tr>
              ...
              <td>
                <!-- <?php if (hasPermissions('permissions_edit')): ?> -->
                  <a href="<?php echo url('KSD/edit/'.$row->id) ?>" class="btn btn-sm btn-default" title="<?php echo lang('edit_permission') ?>" data-toggle="tooltip"><i class="fas fa-edit"></i></a>
                <!-- <?php endif ?> -->
                <!-- <?php if (hasPermissions('permissions_delete')): ?> -->
                  <a href="<?php echo url('KSD/delete/'.$row->id) ?>" class="btn btn-sm btn-default" onclick='return confirm("Do you really want to delete this permissions ? \nIt may cause errors where it is currently being used !!")' title="<?php echo lang('delete_permission') ?>" data-toggle="tooltip"><i class="fa fa-trash"></i></a>
                <!-- <?php endif ?> -->
              </td>
            </tr>
          <?php endforeach ?>
...
window.updateKategoriStatus = (id, status) => {
  $.get( '<?php echo url('KSD/change_status') ?>/'+id, {
    status: status
  }, (data, status) => {
    if (data=='done') {
      // code
