...
class Strakom_model extends MY_Model {
	...
	{
		$query = $this->db->query("SELECT *,tbl_strakom_unggulan.id as strakom_id, tbl_strakom_unggulan.status as status_strakom, (select COUNT(tbl_editorial_plan.id) from tbl_editorial_plan where tbl_editorial_plan.strakom_id=tbl_strakom_unggulan.id AND tbl_editorial_plan.status = 2) as EditorialApprove, (select COUNT(tbl_mitigasi.id) from tbl_mitigasi where tbl_mitigasi.strakom_id=tbl_strakom_unggulan.id AND tbl_mitigasi.status = 2) as MitigasiApprove from tbl_strakom_unggulan join tbl_periode on tbl_periode.id = tbl_strakom_unggulan.periode_id where tbl_periode.status_periode = 1 AND tbl_strakom_unggulan.status = '".$status."'")->result()	;
